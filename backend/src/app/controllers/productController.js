...
const productController = {
    createProduct: async(req, res)=>{
        try {
            ...
            const {name, description, category, colors, images} = req.body
    ...
    getAllProductsByCategoryName: async(req, res)=> {
        try {
          ...
          const products = await Product.find({ category: category.name }).limit(limit)
    ...
    getProductPerpage: async (req, res)=>{
        try {
            ...
            if(search){
                ...
                products = await Product.find({ $or: query }).limit(limit)
            }
                else{
